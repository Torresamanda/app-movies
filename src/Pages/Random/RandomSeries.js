...
import { imgURL, APIKey, TOP_TV, MOVIE_API, } from '../../Config/key'

import favicon from '../../Img/faviconTwo.png'

export default function RandomMovies() {
    const randomPages = Math.floor(Math.random() * 200) + 1

    const [movie, setMovie] = useState({ name: 'Carregando Série' })
    ...
    const getMovies = async event => {
        ...
        const { data } = await axios.get(`${TOP_TV}`, {
            params: {
                api_key: APIKey,
                language: 'pt-BR',
                page: randomPages
        ...
        <>
            <Main>
                ...
                <SectionDetails>
                    ...
                    <Header>
                        <H1>{movie?.name ? movie?.name : 'Não há nome para essa série.'}</H1>
                        <VoteAverange>{parseInt(movie?.vote_average ? movie?.vote_average : '0')}</VoteAverange>
                    </Header>


                    <Sinopse>{movie?.overview ? movie?.overview : 'Desculpe, não há sinopse.'}</Sinopse>

                    <RealeseDate>Data de lançamento: {movie?.first_air_date ? movie?.first_air_date : 'Sem data de lançamento' }</RealeseDate>
